...
test_that("pred must be numeric", {
	expect_error(
		pinterval_parametric(pred = "abc", calib = pred_cal, calib_truth = truth_cal),
...
test_that("alpha must be single numeric in (0,1)", {
	expect_error(
		pinterval_parametric(pred = pred_test, calib = pred_cal, calib_truth = truth_cal, alpha = 0),
		"pinterval_parametric.*alpha"
	)
	expect_error(
		pinterval_parametric(pred = pred_test, calib = pred_cal, calib_truth = truth_cal, alpha = 1),
		"pinterval_parametric.*alpha"
	)
})
...
test_that("dist must be valid", {
	expect_error(
		pinterval_parametric(pred = pred_test, calib = pred_cal, calib_truth = truth_cal, dist = "invalid_dist"),
		"arg"
	)
})
...
test_that("calib and calib_truth must have same length", {
	expect_error(
		pinterval_parametric(pred = pred_test, calib = pred_cal[1:100], calib_truth = truth_cal[1:50]),
		"same length"
	)
})
...
test_that("output is a tibble with correct columns", {
	result <- pinterval_parametric(
		pred = pred_test, calib = pred_cal, calib_truth = truth_cal, dist = "norm"
	)
...
test_that("lognormal distribution works with positive data", {
	...
	result <- pinterval_parametric(
		pred = pt, calib = pc, calib_truth = tc, dist = "lnorm"
	)
	expect_s3_class(result, "tbl_df")
})
...
test_that("gamma distribution works with positive data", {
	...
	result <- pinterval_parametric(
		pred = pt, calib = pc, calib_truth = tc, dist = "gamma"
	)
	expect_s3_class(result, "tbl_df")
})
...
test_that("single prediction works", {
	result <- pinterval_parametric(
		pred = 1.0, calib = pred_cal, calib_truth = truth_cal, dist = "norm"
	)
...
test_that("warns when pred contains NA", {
	expect_warning(
		pinterval_parametric(pred = c(NA_real_, 1), calib = pred_cal, calib_truth = truth_cal),
		"pred.*NA"
	)
})
